...
export const SimpleTracker = () => {
	...
	return (
		<section className="relative merriweather-light">
			...
			<div className="flex flex-col items-center justify-center gap-1 merriweather-light w-screen text-xs">
					...
					return (
						<div
							...
							className={classNames(
								"flex items-center w-full justify-center gap-2 px-2 py-px",
								{
									border: isToday,
								}
							)}>
							<p className="w-4 h-8 font-mono dark:bg-gray-800 flex items-center">
								{index + 1}
							</p>
							<p className="w-6 h-8 dark:bg-gray-800 flex items-center">
								{dayName}
							</p>
							<input
								type="text"
							...
							{trackerData[index].checkboxes.map((checked, i) => (
								<input
									...
									checked={checked}
									onChange={(e) =>
										handleInputChange(index, i, e.target.checked)
									}
								/>
							))}
							<input
